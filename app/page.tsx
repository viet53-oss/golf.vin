...
const MENU_ITEMS: MenuItem[] = [
    {
        ...
        href: "/live",
        color: "text-red-500",
        description: "(Is isolated from club rounds, so play all you want.)"
    },
...
const MenuCard = ({ item, featured = false }: MenuCardProps) => {
...
export default async function Home1() {
    ...
    const featuredItem = MENU_ITEMS.find(item => item.name === "Live Score");
    const regularItems = MENU_ITEMS.filter(item => item.name !== "Live Score");
            ...
            <main className="relative z-10 flex-1 flex flex-col items-center justify-start text-center px-1 pb-10 w-full max-w-full pt-10">
                ...
                <section className="mb-12 space-y-4 w-full">
                    ...
                    <div className="flex flex-col gap-1 text-shadow-md w-full">
                        <p className="text-white text-[18pt] font-extrabold drop-shadow-md leading-relaxed w-full uppercase tracking-wide">
                            Greater New Orleans Golf Courses:
                        </p>
                        <p className="text-white/90 text-[18pt] mt-2 drop-shadow-md font-semibold w-full leading-tight px-4">
                            {GOLF_COURSES.join(", ")}
                        </p>
                    </div>
                </section>

                {/* Featured Menu Item - Live Score */}
                {featuredItem && (
                    <div className="w-full flex justify-center mb-2 px-1">
                        <MenuCard item={featuredItem} featured />
                    </div>
                )}
