...
export async function POST() {
    try {
        console.log('🏌️ Adding Timberlane Golf - Gretna...');

        // Create Timberlane Golf course
        const course = await prisma.course.create({
            data: {
                name: "Timberlane Golf",
            }
        });

        // Create tee boxes
        const teeBoxes = [
            { name: "White", rating: 69.8, slope: 125, par: 72 },
            { name: "Purple", rating: 64.9, slope: 109, par: 72 },
            { name: "Gold", rating: 67.8, slope: 120, par: 72 },
            { name: "Blue", rating: 71.4, slope: 128, par: 72 },
        ];
        ...
        return NextResponse.json({
            success: true,
            message: 'Timberlane Golf added successfully!',
            data: {
                course: course.name,
                teeBoxes: 4,
                holes: 18,
