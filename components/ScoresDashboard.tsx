    ...
    const handleCopyRound = async (round: RoundWithPlayers) => {
        ...
        flights.forEach((flight, index) => {
            ...
            flight.players.forEach((rp, idx) => {
                ...
                if (typeof idxAfter === 'number') {
                    const calc = Math.round((idxAfter * (slope / 113)) + (rating - par));
                    if (!isNaN(calc)) courseHandicapAfter = calc;
                }
                            ...
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    <div className="flex items-center gap-3">
                                        <TrophyIcon className={`w-7 h-7 ${round.is_tournament ? 'text-yellow-500' : 'text-slate-300'}`} />
                                        <h2 className="font-black text-black text-[16pt] tracking-tight">
                                            {dateStr}
                                        </h2>
                                        {round.is_tournament && (
                                            <span className="text-[16pt] font-black text-amber-700 bg-amber-100 px-3 py-1 rounded border border-amber-200 uppercase tracking-widest leading-none">
                                                Tournament
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    {round.name && (
                                        <div className="mt-1">
                                            <div className="text-blue-600 font-bold text-[16pt] tracking-[0.15em] uppercase border border-slate-100 px-2 py-1 rounded inline-block bg-slate-50/50">
                                                {round.name}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {isAdmin && (
                                        <>
                                            ...
                                            <Link
                                                ...
                                                className="bg-black text-white text-[16pt] font-bold uppercase tracking-wider px-4 py-2 rounded-full hover:bg-gray-800 transition-all shadow-lg active:scale-95 ml-2"
                                            >
                                                Edit Round
                                            </Link>
                                        </>
                        ...
                        <div className="overflow-x-auto">
                            {(() => {
                                ...
                                if (round.is_tournament) {
                                    ...
                                    // Sort using the robust tie-breaker logic defined in parent scope
                                    flight1Players.sort(comparePlayers);
                                    flight2Players.sort(comparePlayers);
                                    ...
                                    <div key={fIdx} className={fIdx > 0 ? 'border-t-8 border-slate-50' : ''}>
                                        {flight.name && (
                                            <div className="px-3 py-3 bg-slate-50/50 border-b border-slate-100">
                                                <h3 className="text-[16pt] font-black text-slate-800 flex items-center gap-2 uppercase tracking-widest">
                                        ...
                                        <table className="w-full text-sm">
                                            <thead className="text-[16pt] text-black uppercase font-black tracking-[0.1em] border-b border-slate-100 bg-white">
                                                <tr>
                                            ...
                                            <tbody className="divide-y divide-slate-50">
                                                {flight.players.map((rp, pIdx) => {
                                                    ...
                                                    let courseHandicapAfter = courseHandicap;
                                                    if (typeof idxAfter === 'number') {
                                                        const calculated = Math.round((idxAfter * (slope / 113)) + (rating - par));
                                                        if (!isNaN(calculated)) courseHandicapAfter = calculated;
                                                    }
                                                    ...
                                                    return (
                                                        <tr key={rp.id} className="hover:bg-slate-50/50 transition-colors group">
                                                            <td className="px-3 py-3 text-center text-black font-bold text-[16pt]">{pIdx + 1}</td>
                                                            <td className="px-3 py-3">
                                                                ...
                                                                >
                                                                    <span className="font-bold text-blue-600 text-[16pt] underline decoration-2 decoration-red-600 group-hover/name:text-blue-700 group-hover/name:decoration-red-700 transition-colors uppercase leading-tight">
                                                                        {firstName}
                                                                    </span>
                                                                    <span className="text-[16pt] text-black font-medium leading-none">
                                                                        {lastName}
                                                                    </span>
                                                            ...
                                                            {round.is_tournament && (
                                                                <>
                                                                    <td className="px-3 py-3 text-center">
                                                                        <span className="font-black text-green-600 text-[16pt]">
                                                                            {pIdx === 0 ? '$35' : pIdx === 1 ? '$25' : pIdx === 2 ? '$15' : '-'}
                                                                        </span>
                                                                    </td>
                                                                    <td className="px-3 py-3 text-center">
                                                                        <span className="font-bold text-slate-500 text-[16pt]">
                                                                            +{pIdx === 0 ? '100' : pIdx === 1 ? '75' : pIdx === 2 ? '50' : '20'}
                                                                        </span>
                                                                    </td>
                                                                    <td className="px-3 py-3 text-center">
                                                                        <span className="font-bold text-blue-600 text-[16pt]">
                                                                            {rp.ytdPoints || '-'}
                                                                        </span>
                                                                    </td>
                                                                </>
                                                            )}
                                                            <td className="px-3 py-3 text-center font-bold text-black text-[16pt]">{gross ?? '-'}</td>
                                                            <td className="px-3 py-3 text-center font-bold text-black text-[16pt]">{courseHandicap}</td>
                                                            <td className="px-3 py-3 text-center bg-slate-50/50">
                                                                <span className="font-black text-blue-600 text-[16pt]">
                                                                    {net ?? '-'}
                                                                </span>
                                                            </td>
                                                            <td className="px-3 py-3 text-center">
                                                                <div className="flex items-center justify-center gap-1 leading-none">
                                                                    <span className={`text-[16pt] font-black ${hcpColor}`}>{courseHandicapAfter}</span>
                                                                </div>
                                                            </td>
                                                            <td className="px-3 py-3 text-center">
                                                                <div className="flex items-center justify-center gap-1 leading-none">
                                                                    <span className={`text-[16pt] font-black ${idxColor}`}>{(idxAfter ?? idxBefore).toFixed(1)}</span>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    );
